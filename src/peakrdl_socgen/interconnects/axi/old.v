...
module    axi_interconnect #(
        ...
        // {{{
        input    wire    [N_MASTERS*ID_WIDTH-1:0]        s_awid,
        ...
        input    wire    [N_MASTERS-1:0]            s_awlock,
        input    wire    [N_MASTERS*4-1:0]            s_awcache,
        input    wire    [N_MASTERS*3-1:0]            s_awprot,
        input    wire    [N_MASTERS*4-1:0]            s_awqos,
        input    wire    [N_MASTERS-1:0]            s_awvalid,
        output    wire    [N_MASTERS-1:0]            s_awready,
        //
        input    wire    [N_MASTERS*DATA_WIDTH-1:0]    s_wdata,
        input    wire    [N_MASTERS*DATA_WIDTH/8-1:0]    s_wstrb,
        input    wire    [N_MASTERS-1:0]            s_wlast,
        input    wire    [N_MASTERS-1:0]            s_wvalid,
        output    wire    [N_MASTERS-1:0]            s_wready,
        //
        output    wire    [N_MASTERS*ID_WIDTH-1:0]        s_bid,
        output    wire    [N_MASTERS*2-1:0]            s_bresp,
        output    wire    [N_MASTERS-1:0]            s_bvalid,
        input    wire    [N_MASTERS-1:0]            s_bready,
        // }}}
        // Read slave channels from the controlling AXI masters
        // {{{
        input    wire    [N_MASTERS*ID_WIDTH-1:0]        s_arid,
        ...
        input    wire    [N_MASTERS-1:0]            s_arlock,
        input    wire    [N_MASTERS*4-1:0]            s_arcache,
        input    wire    [N_MASTERS*3-1:0]            s_arprot,
        input    wire    [N_MASTERS*4-1:0]            s_arqos,
        input    wire    [N_MASTERS-1:0]            s_arvalid,
        output    wire    [N_MASTERS-1:0]            s_arready,
        //
        output    wire    [N_MASTERS*ID_WIDTH-1:0]        s_rid,
        output    wire    [N_MASTERS*DATA_WIDTH-1:0]    s_rdata,
        output    wire    [N_MASTERS*2-1:0]            s_rresp,
        output    wire    [N_MASTERS-1:0]            s_rlast,
        ...
        // {{{
        output    wire    [N_SLAVES*ID_WIDTH-1:0]        m_awid,
        output    wire    [N_SLAVES*ADDR_WIDTH-1:0]    m_awaddr,
        output    wire    [N_SLAVES*8-1:0]            m_awlen,
        output    wire    [N_SLAVES*3-1:0]            m_awsize,
        output    wire    [N_SLAVES*2-1:0]            m_awburst,
        output    wire    [N_SLAVES-1:0]            m_awlock,
        output    wire    [N_SLAVES*4-1:0]            m_awcache,
        output    wire    [N_SLAVES*3-1:0]            m_awprot,
        output    wire    [N_SLAVES*4-1:0]            m_awqos,
        output    wire    [N_SLAVES-1:0]            m_awvalid,
        input    wire    [N_SLAVES-1:0]            m_awready,
        //
        //
        output    wire    [N_SLAVES*DATA_WIDTH-1:0]    m_wdata,
        output    wire    [N_SLAVES*DATA_WIDTH/8-1:0]    m_wstrb,
        output    wire    [N_SLAVES-1:0]            m_wlast,
        output    wire    [N_SLAVES-1:0]            m_wvalid,
        input    wire    [N_SLAVES-1:0]            m_wready,
        //
        input    wire    [N_SLAVES*ID_WIDTH-1:0]        m_bid,
        input    wire    [N_SLAVES*2-1:0]            m_bresp,
        input    wire    [N_SLAVES-1:0]            m_bvalid,
        output    wire    [N_SLAVES-1:0]            m_bready,
        // }}}
        // Read channel master outputs to the connected AXI slaves
        // {{{
        output    wire    [N_SLAVES*ID_WIDTH-1:0]        m_arid,
        output    wire    [N_SLAVES*ADDR_WIDTH-1:0]    m_araddr,
        output    wire    [N_SLAVES*8-1:0]            m_arlen,
        output    wire    [N_SLAVES*3-1:0]            m_arsize,
        output    wire    [N_SLAVES*2-1:0]            m_arburst,
        output    wire    [N_SLAVES-1:0]            m_arlock,
        output    wire    [N_SLAVES*4-1:0]            m_arcache,
        output    wire    [N_SLAVES*4-1:0]            m_arqos,
        output    wire    [N_SLAVES*3-1:0]            m_arprot,
        output    wire    [N_SLAVES-1:0]            m_arvalid,
        input    wire    [N_SLAVES-1:0]            m_arready,
        //
        //
        input    wire    [N_SLAVES*ID_WIDTH-1:0]        m_rid,
        input    wire    [N_SLAVES*DATA_WIDTH-1:0]    m_rdata,
        input    wire    [N_SLAVES*2-1:0]            m_rresp,
        input    wire    [N_SLAVES-1:0]            m_rlast,
        input    wire    [N_SLAVES-1:0]            m_rvalid,
        output    wire    [N_SLAVES-1:0]            m_rready
        // }}}
        // }}}
    );
    ...
    // Verilator lint_off UNUSED
    reg    [NSFULL-1:0]    m_awvalid;
    reg    [NSFULL-1:0]    m_awready;
    reg    [IW-1:0]    m_awid    [0:NSFULL-1];
    reg    [7:0]        m_awlen    [0:NSFULL-1];

    reg    [NSFULL-1:0]    m_wvalid;
    reg    [NSFULL-1:0]    m_wready;
    reg    [NSFULL-1:0]    m_bvalid;
    reg    [NSFULL-1:0]    m_bready;
    // Verilator lint_on  UNUSED
    reg    [1:0]        m_bresp    [0:NSFULL-1];
    reg    [IW-1:0]    m_bid    [0:NSFULL-1];

    // Verilator lint_off UNUSED
    reg    [NSFULL-1:0]    m_arvalid;
    reg    [7:0]        m_arlen    [0:NSFULL-1];
    reg    [IW-1:0]    m_arid    [0:NSFULL-1];
    reg    [NSFULL-1:0]    m_arready;
    // Verilator lint_on  UNUSED
    reg    [NSFULL-1:0]    m_rvalid;
    // Verilator lint_off UNUSED
    reg    [NSFULL-1:0]    m_rready;
    // Verilator lint_on  UNUSED
    //
    reg    [IW-1:0]    m_rid    [0:NSFULL-1];
    reg    [DW-1:0]    m_rdata    [0:NSFULL-1];
    reg    [NSFULL-1:0]    m_rlast;
    reg    [2-1:0]        m_rresp    [0:NSFULL-1];
    ...
    // m_* convenience signals (write side)
    // {{{
    always @(*)
    begin
        m_awvalid = -1;
        ...
        m_awvalid[N_SLAVES-1:0] = m_awvalid;
        m_awready[N_SLAVES-1:0] = m_awready;
        m_wvalid[N_SLAVES-1:0]  = m_wvalid;
        m_wready[N_SLAVES-1:0]  = m_wready;
        m_bvalid[N_SLAVES-1:0]  = m_bvalid;
        m_bready[N_SLAVES-1:0]  = m_bready;

        for(iM=0; iM<N_SLAVES; iM=iM+1)
        begin
            m_awid[iM]   = m_awid[   iM*IW +: IW];
            m_awlen[iM]  = m_awlen[  iM* 8 +:  8];

            m_bid[iM]   = m_bid[iM* IW +:  IW];
            m_bresp[iM] = m_bresp[iM* 2 +:  2];

            m_rid[iM]   = m_rid[  iM*IW +: IW];
            m_rdata[iM] = m_rdata[iM*DW +: DW];
            m_rresp[iM] = m_rresp[iM* 2 +:  2];
            m_rlast[iM] = m_rlast[iM];
        end
        ...
        begin
            ...
            m_bresp[iM] = INTERCONNECT_ERROR;
            m_bid[iM]   = 0;

            m_rid[iM]   = 0;
            m_rdata[iM] = 0;
            m_rresp[iM] = INTERCONNECT_ERROR;
            m_rlast[iM] = 1;
        end
    end
    // }}}

    // m_* convenience signals (read side)
    // {{{
    always @(*)
    begin
        m_arvalid = 0;
        ...
        for(iM=0; iM<N_SLAVES; iM=iM+1)
        begin
            m_arlen[iM] = m_arlen[iM* 8 +:  8];
            m_arid[iM]  = m_arid[ iM*IW +: IW];
        end
        for(iM=N_SLAVES; iM<NSFULL; iM=iM+1)
        begin
            m_arlen[iM] = 0;
        ...
        m_arvalid[N_SLAVES-1:0] = m_arvalid;
        m_arready[N_SLAVES-1:0] = m_arready;
        m_rvalid[N_SLAVES-1:0]  = m_rvalid;
        m_rready[N_SLAVES-1:0]  = m_rready;
    end
    // }}}
    ...
    begin
        ...
        slave_awready[N_SLAVES-1:0] = (~m_awvalid | m_awready);
        slave_wready[N_SLAVES-1:0]  = (~m_wvalid | m_wready);
        slave_arready[N_SLAVES-1:0] = (~m_arvalid | m_arready);
    end
    // }}}
    ...
    // {{{
        ...
        skidbuffer #(
            ...
            // {{{
            clk, !rstn,
            s_awvalid[N], S_AWREADY[N],
            { s_awid[N*IW +: IW], s_awaddr[N*AW +: AW],
              s_awlen[N*8 +: 8], s_awsize[N*3 +: 3],
              s_awburst[N*2 +: 2], s_awlock[N],
              s_awcache[N*4 +: 4], s_awprot[N*3 +: 3],
              s_awqos[N*4 +: 4] },
        ...
        skidbuffer #(
            ...
            // {{{
            clk, !rstn,
            s_wvalid[N], s_wready[N],
            { s_wdata[N*DW +: DW], s_wstrb[N*DW/8 +: DW/8],
              s_wlast[N] },
    ...
    // {{{
        ...
        skidbuffer #(
            ...
            // {{{
            clk, !rstn,
            s_arvalid[N], s_arready[N],
            { s_arid[N*IW +: IW], s_araddr[N*AW +: AW],
              s_arlen[N*8 +: 8], s_arsize[N*3 +: 3],
              s_arburst[N*2 +: 2], s_arlock[N],
              s_arcache[N*4 +: 4], s_arprot[N*3 +: 3],
              s_arqos[N*4 +: 4] },
    ...
    // {{{
        ...
        always @(*)
            sawstall= (m_awvalid[M]&& !m_awready[M]);
        ...
        initial    axi_awqos   = 0;
        always @(posedge  clk)
        if (OPT_LOWPOWER && (!rstn || !swgrant[M]))
        begin
        ...
        // {{{
        assign    m_awvalid[M]          = axi_awvalid;
        assign    m_awid[   M*IW +: IW] = axi_awid;
        assign    m_awaddr[ M*AW +: AW] = axi_awaddr;
        assign    m_awlen[  M* 8 +:  8] = axi_awlen;
        assign    m_awsize[ M* 3 +:  3] = axi_awsize;
        assign    m_awburst[M* 2 +:  2] = axi_awburst;
        assign    m_awlock[ M]          = axi_awlock;
        assign    m_awcache[M* 4 +:  4] = axi_awcache;
        assign    m_awprot[ M* 3 +:  3] = axi_awprot;
        assign    m_awqos[  M* 4 +:  4] = axi_awqos;
        //
        //
        assign    m_wvalid[M]             = axi_wvalid;
        assign    m_wdata[M*DW +: DW]     = axi_wdata;
        assign    m_wstrb[M*DW/8 +: DW/8] = axi_wstrb;
        assign    m_wlast[M]              = axi_wlast;
        //
        //
        assign    m_bready[M]             = axi_bready;
        // }}}
        //
    // }}}
    end endgenerate
    ...
    // {{{
        ...
        always @(*)
            arstall= axi_arvalid && !m_arready[M];
        ...
        initial axi_arqos   = 0;
        always @(posedge  clk)
        if (OPT_LOWPOWER && (!rstn || !srgrant[M]))
        begin
            axi_arid    <= 0;
            axi_araddr  <= 0;
            axi_arlen   <= 0;
        ...
        //
        assign    m_arvalid[M]          = axi_arvalid;
        assign    m_arid[   M*IW +: IW] = axi_arid;
        assign    m_araddr[ M*AW +: AW] = axi_araddr;
        assign    m_arlen[  M* 8 +:  8] = axi_arlen;
        assign    m_arsize[ M* 3 +:  3] = axi_arsize;
        assign    m_arburst[M* 2 +:  2] = axi_arburst;
        assign    m_arlock[ M]          = axi_arlock;
        assign    m_arcache[M* 4 +:  4] = axi_arcache;
        assign    m_arprot[ M* 3 +:  3] = axi_arprot;
        assign    m_arqos[  M* 4 +:  4] = axi_arqos;
        //
        assign    m_rready[M]          = axi_rready;
        //
    // }}}
    end endgenerate
    // }}}
    ...
    // {{{
        ...
        if (berr_valid[N])
            ...
            bskd_valid[N] = mwgrant[N]&&m_bvalid[mwindex[N]];
        ...
        begin
            i_bid   = berr_id[N];
            i_bresp = INTERCONNECT_ERROR;
        end else begin
            i_bid   = m_bid[mwindex[N]];
            i_bresp = m_bresp[mwindex[N]];
        end
        // }}}
        ...
        skidbuffer #(
            ...
            { i_bid, i_bresp },
            s_bvalid[N], s_bready[N],
            { s_bid[N*IW +: IW], s_bresp[N*2 +: 2] }
            // }}}
        );
        // }}}
    // }}}
    ...
    // {{{
        ...
        if (rgrant[N][N_SLAVES])
            ...
            rskd_valid[N] = mrgrant[N] && m_rvalid[mrindex[N]];
        ...
        begin
            ...
            i_rresp = INTERCONNECT_ERROR;
        end else begin
            i_rid   = m_rid[mrindex[N]];
            i_rdata = m_rdata[mrindex[N]];
            rskd_rlast[N]= m_rlast[mrindex[N]];
            i_rresp = m_rresp[mrindex[N]];
        end
        // }}}
        ...
        skidbuffer #(
            ...
            { i_rid, i_rdata, rskd_rlast[N], i_rresp },
            s_rvalid[N], s_rready[N],
            { s_rid[N*IW +: IW], s_rdata[N*DW +: DW],
              s_rlast[N], s_rresp[N*2 +: 2] }
            // }}}
        );
        // }}}
    // }}}
